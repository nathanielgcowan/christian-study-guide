...
import { ArrowRight, BrainCircuit, Compass, Sparkles, Target, Users } from "lucide-react";
...
const recommendationCards = [
  {
    title: "Next passage",
    detail: "Recommend the next Scripture based on what the user has been reading and saving recently.",
    icon: Compass,
  },
  {
    title: "Next doctrine",
    detail: "Surface the theology topic or life issue that naturally follows the current study pattern.",
    icon: BrainCircuit,
  },
  {
    title: "Next habit",
    detail: "Suggest whether the user should read, pray, review a memory verse, or resume a path next.",
    icon: Target,
  },
  {
    title: "Next community step",
    detail: "Help the user move from solo study into prayer follow-up, groups, and encouragement.",
    icon: Users,
  },
];
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (isSignedIn) {
          const [activity, journeys, preferences, prayers, progress, sessions] = await Promise.all([
            getActivityTimeline(),
            getJourneys(),
            getPersonalizationPreferences(),
            getPrayerEntries(),
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="max-w-4xl">
            ...
            <p className="mt-6 text-lg leading-8 text-orange-50">
              This recommendation layer now looks at study continuity, prayers, growth goals,
              journeys, and new believer progress so the app can suggest a real next step.
            </p>
          </div>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-6xl px-6 py-14">
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <aside className="rounded-3xl border border-orange-200 bg-orange-50 p-8">
            <div className="flex items-center gap-3 text-orange-950">
              ...
              <h2 className="text-2xl font-semibold">What shapes these suggestions</h2>
            </div>
            <div className="mt-6 space-y-3 text-sm leading-7 text-orange-950">
              <p>• Your latest study sessions and unfinished passage work</p>
              <p>• Active prayer requests and answered-prayer follow-up opportunities</p>
        ...
        <section className="mt-10 rounded-3xl border border-blue-200 bg-blue-50 p-8">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-7 text-blue-950">
            Instead of sending people into isolated pages, the app now surfaces the most recent
            study, prayer, and journey moments so they can continue what they already started.
          </p>
